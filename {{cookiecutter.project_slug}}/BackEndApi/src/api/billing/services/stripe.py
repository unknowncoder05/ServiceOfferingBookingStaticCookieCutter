...
`stripe` is imported lazily inside each function so that the billing app loads
successfully even if the stripe package is removed from requirements.  All
Stripe calls are guarded by billing_settings.stripe_secret_key; if the key is
absent the callers (views) return a 503 before reaching here.
"""
import logging
...
def handle_checkout_completed(event_data):
    ...
    Idempotent — checks for an existing transaction with the same session ID.
    """
