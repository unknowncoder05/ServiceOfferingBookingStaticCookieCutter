...
def check_user_balance(user):
    ...
    Admin users (is_staff or is_superuser) bypass the balance check
    but still have their usage tracked via deduct_credits.

    Raises InsufficientCreditsError if balance <= minimum.
    """
...
def deduct_credits(
    ...
    Balance can go negative — user must add credits before next action.
    """
    amount = Decimal(str(amount))
    if amount <= 0:
