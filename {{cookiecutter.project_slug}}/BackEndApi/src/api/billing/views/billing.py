...
`stripe` is imported lazily in the webhook handler so the module loads even
when the stripe package is not installed.  The checkout view guards behind
billing_settings.stripe_secret_key, so Stripe is never called unconfigured.
"""
import logging
